...
const UpdatePieceForm = ({ initialValues, handleClose, getPieces }) => {
  ...
  const handleUpdatePiece = async (values, onSubmitProps) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await HTTP.patch(`/pieces/update/${id}?_updatedBy=${localStorage.getItem('username')}`, values)
          .then((response) => {
            getPieces();
            Swal.fire({
              position: "top-end",
...
export default UpdatePieceForm;
