...
const QandelBasStore = () => {
  ...
  const getAllDatas = async () => {
    setLoading(true);
    await HTTP.get(`/qandelblbasstore/grouped?_search=${search}&_date=${date}`)
      .then((response) => {
        setLoading(false);
  ...
  const getSentDatas = async () => {
    setLoading(true);
    await HTTP.get(
      `/sent/all?_from=QandelBlbas&_limit=${slimit}&_page=${scurrentPage}&_search=${ssearch}`
    )
      .then((response) => {
        setSLoading(false);
  ...
  useEffect(() => {
    getAllDatas();
    getSentDatas();
  }, [search, date, slimit, scurrentPage, ssearch]);
  return (
    <>
      {/* ADD Prduction */}
      ...
      <Row className="mt-5">
        <Col md={4}>
          <h4 className="mb-3">{t("qandelblblas_store")} </h4>
        </Col>
        <Col md={2}>
          {role == "Admin" || role == "Accountant" || role == "QandelBlbas" ? (
            <Button
