...
const AllSelleds = () => {
  ...
  const [selledLength, setSelledLength] = useState(0);
  ...
  const getSentDatas = async () => {
    setLoading(true);
    await HTTP.get(
      `/sent/all?_limit=${limit}&_page=${currentPage}&_search=${search}&_date=${date}&_selled=${1}`
    )
      .then((response) => {
        setLoading(false);
        setCount(Math.ceil(response.data.count / limit));
        setSelledLength(response.data.count);
        setSelled(response.data.rows);
      })
      .catch((error) => {
        setLoading(false);
        setSelled([]);
  ...
  return (
    <>
      ...
      >
        <h5 className="d-on-print">{t("all_sold")}</h5>
        <Table
